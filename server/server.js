...
io.on('connection', socket => {
    ...
    socket.on("clear_board", data => {
        ...
        io.to(data.roomId).emit("receive_clear_board");
    });
    ...
    socket.on("start_game", data => {
        ...
        socket.to(data.roomId).emit("receive_start_game", room);
    });

    socket.on("update_round", data => {
        const room = getRoom(data.roomId);
        room.currentRound++;
        io.to(data.roomId).emit("receive_update_round", { currentRound: room.currentRound });
    });

    socket.on("new_drawer", data => {
        const room = getRoom(data.roomId);
        room.currentDrawer = room.users.find(u => u.id === data.newDrawer.id);

        console.log("new drawer: ", room.currentDrawer.username);
        socket.to(data.roomId).emit("receive_new_drawer", { newDrawer: room.currentDrawer })
    });
